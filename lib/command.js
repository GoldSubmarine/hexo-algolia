  ...
  Promise.resolve(apiKey)
    ...
    .then(function(publishedPagesAndPosts) {
      return publishedPagesAndPosts.map(function(data) {
        ...
        if(data.content && (Array.isArray(data.content) || typeof data.content.toArray === 'function')) {
          storedPost.content = striptags(data.content);
          storedPost.content = entities.decode(data.content);
        }
