...
module.exports = function(args, callback) {
  ...
  Promise.resolve(apiKey)
    ...
    .then(function(publishedPosts) {
      var result = []
      publishedPosts.map(function(data) {
        let content = striptags(data.content);
        content = entities.decode(content);
        let size = 2000
        for(let i = 0; i < content.length/size; i++) {
          let tmpData = parse(stringify(data))
          tmpData.chunkIndex = i;
          tmpData.content = content.slice(i*size, (i+1)*size);
          result.push(tmpData)
        }
      })
      return result
    ...
    .then(function(publishedPagesAndPosts) {
      return publishedPagesAndPosts.map(function(data) {
        ...
        storedPost.objectID = computeSha1(data.path+data.chunkIndex);
